 ...
 *
 * <code>mvn exec:java</code> to start the server.
 * <code>curl -i http://localhost:8080/vhf/stations/list</code>
 * to execute a client
 * See also the scripts in the bin directory, e.g. <code>client.sh</code>
 *
 * This object is in charge of the public API : it processes requests
 * and responses to clients.
 *
 * The serialization and the deserialization of objects is carried out in
 * the RestController object.
 *
 * Some responses may be cached. The current implementation is experimental
 * and must be revisited later on.
 *
 * The data model is managed by the StationManager object.
 *
 * This first Rest API is almost completely correct. Really ?
 */
@Path("stations")
public class StationService {
    ...
    public StationService() {
        ...
        logger.fine("StationService created");
        ...
        if (level != null && level.intValue() < level.CONFIG.intValue()) {
            debug = 1;
        }

        logger.fine("Level: " + debug);
    }
    //__________________________________________________________________________
    /**
    ...
    public final Response putData(String stationData,
        ...
        try {
            ...
            if (qname == null || !qname.equals('"' + name + '"')) {
                logger.warning("putData is " + name);
                return Response.status(Response.Status.BAD_REQUEST)
