...
# Jean-Paul Le Fèvre - September 2015
#______________________________________________________________________________

# Try with -u jaypee -m m2p or export MY_LOGIN="jaypee:m2p"

usage="Usage: client.sh [-h] -u user -m pw"
options="
[-i][-l|L][-n][-p file][-P file][-g name][-G name][-d name]"
readme="See also the README.md"
...
curl="curl -i --cacert $SRV_HOME/etc/rest-server.pem"
...
case $1 in
    ...
    -g)
            if [ -z "$2" ]; then
                ...
                $curl -H "Accept: application/json" $login "$root/$2"
                echo
                shift
            fi
            shift
